...
import rawInsights from './data/insights.json'; // keep your file at app/data/insights.json

// --- Types aligned to your JSON structure ---
interface Insight {
  id: number;
  date?: string;               // e.g., "23-10-2025" (optional)
  category: string;            // e.g., "Economics", "Equities", etc.
  categoryColor?: string;      // Tailwind bg-* class, optional
  title: string;
  summary?: string;            // short text
  fullContent?: string;        // long text
  chartPath?: string;          // relative path to image/svg
  chartHeight?: string;        // optional custom height (e.g., "420px")
}
...
export default function Page() {
  ...
  const categories = React.useMemo(() => {
    ...
    (rawInsights as Insight[]).forEach(i => set.add(i.category));
